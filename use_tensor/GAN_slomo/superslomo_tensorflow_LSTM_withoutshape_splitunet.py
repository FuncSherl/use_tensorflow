...
class SuperSlomo:
    def __init__(self, sess):
        ...
        with tf.variable_scope("first_unet",  reuse=tf.AUTO_REUSE) as scope:
            ...
            #self.first_opticalflow=my_unet_split( firstinput, 4,training=self.training , withbias=True, withbn=False)
            self.first_opticalflow=my_unet_part_split( firstinput, 4,training=self.training , withbias=True, withbn=False)
            ...
            #self.first_opticalflow_eval=my_unet_split( firstinput, 4,training=self.training , withbias=True, withbn=False)  #注意这里是直接作为optical flow
            self.first_opticalflow_eval=my_unet_part_split( firstinput, 4,training=self.training , withbias=True, withbn=False)  #注意这里是直接作为optical flow
        ...
        #训练G的总loss
        self.G_loss_all=180 * self.second_L1_loss_interframe + 65 *  self.first_warp_loss  + 100 * self.second_contex_loss \
                    +self.second_global_var_loss_all + self.second_GAN_loss_mean_D1* (  tf.sqrt( self.global_step*1.0/maxstep )  )
